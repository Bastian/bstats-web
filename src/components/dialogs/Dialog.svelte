...
<Modal {open} on:close={close}>
    <div class="bg-white dark:bg-gray-800 px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
        <div class="sm:flex sm:items-start">
            ...
            >
                {#if type == "error"}
                    <Exclamation class="h-6 w-6 text-red-600 dark:text-red-100"/>
                {:else if type == "warn"}
                    <Exclamation class="h-6 w-6 text-yellow-500 dark:text-yellow-100"/>
                {/if}
            </div>
    ...
    <div class="bg-gray-50 dark:bg-gray-900 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
        <Button on:click={close} class="w-full sm:w-auto">
            Okay
        </Button>
    </div>
</Modal>
