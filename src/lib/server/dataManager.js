...
function getPluginById() {
	...
	let fields =
		arguments.length === 3 ? arguments[1] : ['name', 'software', 'charts', 'owner', 'global'];
	let callback = arguments.length === 3 ? arguments[2] : arguments[1];
...
function getPluginByIdAsPromise() {
	return new Promise((resolve, reject) => {
		let id = parseInt(arguments[0]);
		let fields =
			arguments.length === 2 ? arguments[1] : ['name', 'software', 'charts', 'owner', 'global'];
...
function getPluginBySoftwareUrlAndName() {
	...
	let fields =
		arguments.length === 4 ? arguments[2] : ['name', 'software', 'charts', 'owner', 'global'];
	let callback = arguments.length === 4 ? arguments[3] : arguments[2];
	databaseManager
		.getRedisCluster()
		.get(`plugins.index.id.url+name:${url}.${name}`, function (err, res) {
			if (err) {
				callback(err, null);
				return;
			}
...
function getPluginBySoftwareUrlAndNameAsPromise() {
	return new Promise((resolve, reject) => {
		...
		let name = arguments[1].toLowerCase();
		let fields =
			arguments.length === 3 ? arguments[2] : ['name', 'software', 'charts', 'owner', 'global'];
...
function getChartByUid() {
	...
	let fields =
		arguments.length === 3 ? arguments[1] : ['id', 'type', 'position', 'title', 'default', 'data'];
	let callback = arguments.length === 3 ? arguments[2] : arguments[1];
...
function getChartByPluginIdAndChartId() {
	...
	let fields =
		arguments.length === 4 ? arguments[2] : ['id', 'type', 'position', 'title', 'default', 'data'];
	let callback = arguments.length === 4 ? arguments[3] : arguments[2];
	databaseManager
		.getRedisCluster()
		.get(`charts.index.uid.pluginId+chartId:${pluginId}.${chartId}`, function (err, res) {
			if (err) {
				callback(err, null);
				return;
			}
...
function getPluginsOfUser() {
	...
	databaseManager
		.getRedisCluster()
		.smembers(`users.index.plugins.username:${username}`, function (err, pluginIds) {
			if (err) {
				return callback(err, null);
			}
			let promises = [];
...
function getLimitedLineChartData(chartUid, line, amount, callback) {
	...
	databaseManager
		.getRedisCluster()
		.hmget(`data:${chartUid}.${line}`, datesToFetch, function (err, res) {
			if (err) {
				callback(err, null);
				return;
			}
...
function getPieData(chartUid, callback) {
	...
	databaseManager
		.getRedisCluster()
		.zrange(`data:${chartUid}.${tms2000}`, 0, -1, 'WITHSCORES', function (err, res) {
			if (err) {
				callback(err, null);
				return;
			}
...
function getDrilldownPieDrilldownData(chartUid, name, callback) {
	...
	databaseManager
		.getRedisCluster()
		.zrange(`data:${chartUid}.${tms2000}.${name}`, 0, -1, 'WITHSCORES', function (err, res) {
			if (err) {
				callback(err, null);
				return;
			}
...
function updatePieData(chartUid, tms2000, valueName, value) {
	databaseManager
		.getRedisCluster()
		.zincrby(`data:${chartUid}.${tms2000}`, value, valueName, function (err, res) {
			if (err) {
				console.log(err);
				return;
			}
...
function updateLineChartData(chartUid, value, line, tms2000) {
	databaseManager
		...
		.hincrby(
			`data:${chartUid}.${line}`,
			timeUtil.tms2000ToDate(tms2000).getTime(),
			value,
			function (err, res) {
				if (err) {
					console.log(err);
...
function updateDrilldownPieData(chartUid, tms2000, valueName, values) {
	...
	Object.keys(values).forEach(function (key) {
		...
		databaseManager
			.getRedisCluster()
			.zincrby(`data:${chartUid}.${tms2000}.${valueName}`, values[key], key, function (err, res) {
				if (err) {
					console.log(err);
					return;
				}
	...
	databaseManager
		.getRedisCluster()
		.zincrby(`data:${chartUid}.${tms2000}`, totalValue, valueName, function (err, res) {
			if (err) {
				console.log(err);
				return;
			}
...
function addPageToCache(url, content, callback) {
	...
	databaseManager
		.getRedisCluster()
		.set(`urlcache:${tms2000}.${url}`, content, 'EX', 60 * 31, callback);
}
...
export {
	...
	getSoftwareById,
	getPluginById,
	getPluginByIdAsPromise,
	getPluginBySoftwareUrlAndName,
	getPluginBySoftwareUrlAndNameAsPromise,
	getGlobalPluginBySoftwareUrl,
	getChartByUid,
	getChartByPluginIdAndChartId,
	getChartUidByPluginIdAndChartId,
	getAllPluginIds,
	getAllSoftwareIds,
	getAllSoftware,
	getChartsByPluginId,
...
export {
	getLimitedLineChartData,
	getFullLineChartData,
	getPieData,
	getDrilldownPieData,
	getMapData
};
...
export { updatePieData, updateMapData, updateLineChartData, updateDrilldownPieData, updateBarData };

// Methods for the page cache
export { getCachedPage, addPageToCache };
