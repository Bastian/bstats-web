		...
		<ul class="mt-4 space-y-2 text-sm text-slate-600">
			{#each links as { href, label } (href)}
				<li>
					<a
						{href}
						class="-m-1 rounded p-1 transition-colors hover:text-brand-800 focus-visible:ring-2 focus-visible:ring-brand-500/50 focus-visible:outline-none"
					>
						{label}
					</a>
				</li>
			...
			<div class="md:col-span-3 lg:col-span-2">
				<div class="inline-flex items-center gap-3">
					...
					<div>
						...
						<p class="text-sm text-slate-500">Open source metrics since 2016.</p>
					</div>
				</div>
				<p class="mt-6 max-w-xl text-sm leading-relaxed text-slate-600">
					bStats started as a fun side project for my own plugins and grew into the de-facto
					standard for Minecraft server plugin metrics.
				</p>
