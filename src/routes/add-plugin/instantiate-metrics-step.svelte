<script lang="ts" module>
    ...
    import instantiateHytale from './_instantiate-hytale.txt?raw';
    ...
    {#if (status === 'active' || status === 'done') && platform}
        ...
        {:else if platform === 'hytale'}
            <div class="space-y-4">
                ...
                <CodeBlock
                    code={processCode(instantiateHytale)}
                    lang="java"
                    transformers={codeTransformers}
                />
            </div>
        {:else if platform === 'server-implementation'}
            <div class="max-w-prose space-y-4">
                <p>
                    If your server implementation is a fork of an existing server implementation on
                    bStats, find the plugin ID and replace it with your own ID in the existing
                    Metrics instantiation code.
                </p>
                <p>
