<script lang="ts" module>
	...
	export const METRICS_VERSION = '3.1.0';
...
<WizardStep index={4} title="Include the Metrics dependency" {status}>
		...
		{#if buildTool === 'copy-and-paste'}
			...
			<div class="doc-callout doc-callout-warning mt-4">
				<p class="mt-2 max-w-prose text-sm text-slate-600">
					You <b>MUST NOT</b> modify the code in any way except for the package name. Doing so will result
					in your plugin being removed from bStats and your account being banned.
				</p>
				<p class="mt-2 max-w-prose text-sm text-slate-600">
			...
			<div class="space-y-4">
				...
				<Collapsible.Root class="w-full">
					<div class="max-w-prose">
						...
						similar to this: <Collapsible.Trigger
	...
	{#if (status === 'active' || status === 'done') && platform && buildTool && platform === 'server-implementation'}
		<div class="max-w-prose space-y-4">
			<p>
				If your server implementation is a fork of a server implementation that already has bStats
				integrated, you don't have to do anything and can continue to the next step.
			</p>
			<p>
				Otherwise, please contact us in our Discord server or via a GitHub issue to consult us how
				to best include bStats Metrics in your server implementation.
				<span class="font-bold">
					Do <span class="text-red-600">NOT</span> try to include bStats on your own without consulting
					us first, or your server implementation will get removed and your account banned
				</span>.
			</p>
			{@render NextButton()}
		</div>
